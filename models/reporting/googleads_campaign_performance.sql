...
pink as leads,
callsfromsmartcampaignads as calls
FROM {{ ref('googleads_performance_by_campaign') }}
LEFT JOIN
    (SELECT DATE_TRUNC('day',date) as date, 'day' as date_granularity, customer_id as account_id, id as campaign_id,
    ...
    SELECT DATE_TRUNC('week',date) as date, 'week' as date_granularity, customer_id as account_id, id as campaign_id,
        COALESCE(SUM(CASE WHEN conversion_action_name = 'Google Adwords - Pink' THEN all_conversions END),0) as pink,
        COALESCE(SUM(CASE WHEN conversion_action_name = 'Google Adwords - Raspberry' THEN all_conversions END),0) as raspberry
    FROM {{ source('googleads_raw','campaign_convtype_performance_report') }}
    GROUP BY 1,2,3,4
    ...
    UNION ALL
    SELECT DATE_TRUNC('month',date) as date, 'month' as date_granularity, customer_id as account_id, id as campaign_id,
        COALESCE(SUM(CASE WHEN conversion_action_name = 'Google Adwords - Pink' THEN all_conversions END),0) as pink,
        COALESCE(SUM(CASE WHEN conversion_action_name = 'Google Adwords - Raspberry' THEN all_conversions END),0) as raspberry
    FROM {{ source('googleads_raw','campaign_convtype_performance_report') }}
    GROUP BY 1,2,3,4
    ...
    SELECT DATE_TRUNC('quarter',date) as date, 'quarter' as date_granularity, customer_id as account_id, id as campaign_id,
        COALESCE(SUM(CASE WHEN conversion_action_name = 'Google Adwords - Pink' THEN all_conversions END),0) as pink,
        COALESCE(SUM(CASE WHEN conversion_action_name = 'Google Adwords - Raspberry' THEN all_conversions END),0) as raspberry
    FROM {{ source('googleads_raw','campaign_convtype_performance_report') }}
    GROUP BY 1,2,3,4
    ...
    SELECT DATE_TRUNC('year',date) as date, 'year' as date_granularity, customer_id as account_id, id as campaign_id,
        COALESCE(SUM(CASE WHEN conversion_action_name = 'Google Adwords - Pink' THEN all_conversions END),0) as pink,
        COALESCE(SUM(CASE WHEN conversion_action_name = 'Google Adwords - Raspberry' THEN all_conversions END),0) as raspberry
    FROM {{ source('googleads_raw','campaign_convtype_performance_report') }}
    GROUP BY 1,2,3,4)
    USING(date, date_granularity, account_id, campaign_id)
